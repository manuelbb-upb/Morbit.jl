...
function initialize_data( mop :: AbstractMOP, x0 :: Vec; 
	...
	x_scaled = ensure_precision(scale( x0, smop ))
	...
	# build SuperDB `sdb`
	groupings = do_groupings( mop, ac )
	if isnothing( populated_db ) 
		sdb, x_index_mapping = build_super_db( groupings, x, eval_res )
	else
		sdb = populated_db 
		transform!( sdb, smop )
	...
	fx, c_e, c_i = eval_result_to_all_vectors( eval_result, mop )
	id = init_iter_data( IterData, x, fx, c_e, c_i, Δ_0 )

    sdb = SuperDB(; sub_dbs, iter_data = typeof(id)[] )
...
function iterate!( iter_data :: AbstractIterData, data_base :: AbstractSuperDB, mop :: AbstractMOP, 
	...
	new_x_indices = put_eval_result_into_db!( sdb, mop_result, x )
...
function optimize( mop :: AbstractMOP, x0 :: Vec;
	...
	mop, iter_data, data_base, sc, ac = initialize_data(mop, x0; algo_config, populated_db)
