struct SurrogateWrapper{
        ...
        EIndType <: Tuple{Vararg{EqConstraintIndex}}, 
        IIndType <: Tuple{Vararg{IneqConstraintIndex}} 
...
get_eq_constraint_indices( sw :: SurrogateWrapper ) = sw.eq_constraint_indices
get_ineq_constraint_indices( sw :: SurrogateWrapper ) = sw.ineq_constraint_indices
...
function init_surrogates( ::Type{<:SurrogateContainer}, mop :: AbstractMOP, 
    ...
    for group in groupings
        meta = prepare_init_model( group.cfg, group.indices, mop, id, sdb, ac; meta_array )
    ...
    for (i,group) in enumerate(groupings)
        _meta = meta_array[i]
        model, meta = init_model( _meta, group.cfg, group.indices, mop, id, sdb, ac )
...
function update_surrogates!( sc :: SurrogateContainer, mop :: AbstractMOP, 
    ...
    for sw in list_of_wrappers( sc )
        ...
        meta = prepare_update_model(
            ...
            mop, id, sdb, ac; ensure_fully_linear, meta_array ) 
        push!(meta_array, meta)
    end
    ...
    for (si,sw) ∈ enumerate( list_of_wrappers(sc ) )
        ...
        new_model, new_meta = update_model( 
            ...
            mop, id, sdb, ac; ensure_fully_linear )
