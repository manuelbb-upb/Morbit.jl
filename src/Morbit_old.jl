...
"""
function initialize_data( mop :: AbstractMOP, x0 :: RVec, fx0 :: Vec = Float32[]; 
    ...
    if isempty( fx0 )
        ...
        fx_sorted = eval_all_objectives( mop, tfn, x_scaled );
    else
    ...
    return (mop, tfn, ac, iter_data, sc)
end

function iterate!( iter_data :: AbstractIterData, mop :: AbstractMOP, tfn :: TransformerFn,
    ...
    mx = eval_models(sc, x);
    fx₊ = eval_all_objectives(mop, tfn, x₊);
...
function optimize( mop :: AbstractMOP, x0 :: Vec, fx0 :: Vec = Float16[];
    ...
    mop, tfn, ac, iter_data, sc = initialize_data( mop, x0, fx0; algo_config, populated_db )

    @logmsg loglevel1 _stop_info_str( ac, mop )

    @logmsg loglevel1 "Entering main optimization loop."
    while true
        abbort = iterate!(iter_data, mop, tfn, sc, ac)
        abbort && break;            
    end# while
